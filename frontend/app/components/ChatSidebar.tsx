...
const ChatSidebar = ({ sidebarOpen, setShowAllUsers, setSidebarOpen, showAllUsers, users, loggedInUser,
    chats, selectedUser, setSelectedUser, handleLogout, createChat, onlineUsers }: ChatSidebarProps) => {
    const [serachQuery, setSerachQuery] = useState("");
    return (
        <aside className={`fixed z-20 sm:static top-0 left-0 h-screen w-80 bg-gray-900 border-r border-gray-700
            transform ${sidebarOpen ? "translate-x-0" : "-translate-x-full"} sm:translate-x-0 transition-transform
            ...
            <div className="flex-1 overflow-hidden px-4 py-2">
                {
                    showAllUsers ? (<div className="space-y-4 h-full">
                        <div className="relative">
                            ...
                            />
                            <input type="text" placeholder='Search Users....' className='w-full pl-10 pr-4 py-3 
                                ...
                                chats.map((chat) => {
                                    ...
                                    return (
                                        <button key={chat.chat._id}
                                            ...
                                            className={`w-full text-left p-4 rounded-lg transition-colors ${isSelected ? "bg-blue-600 border border-blue-500"
                                            ...
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    <div className="flex items-center justify-between mb-1">
                                                        ...
                                                        {
                                                            unseenCount > 0 && <div className="bg-red-600 text-white text-xs font-bold rounded-full min-w-5.5
                                                    ...
                                                    </div>
                                                    {
                                                        latestMessage && <div className="flex items-center gap-2">
                                                            {isSentByMe ? (<CornerUpLeft size={14} className='text-blue-400 text-shrink-0' />) : (<CornerDownRight
                                                                size={14} className='text-green-400 text-shrink-0' />)}
